...
def consultar_consopt(cnpj):
    ...
    try:
        ...
        # ----------------------------------------------------
        # ATENÇÃO:
        # O portal pode apresentar CAPTCHA ou alterar
        # a estrutura da página.
        # Nunca transformar ausência de resultado em "regular".
        # ----------------------------------------------------
        ...
        # Tentativas de identificar informações apresentadas
        # pelo próprio portal.
...
def consultar_pgmei(cnpj, cpf=None, codigo_acesso=None):
    """
    O PGMEI é tratado separadamente.

    A aplicação NÃO presume que o DAS esteja pago.

    Se não houver credenciais/sessão necessária,
    o resultado será NÃO ANALISADO.
    """
    ...
    # --------------------------------------------------------
    # PONTO DE INTEGRAÇÃO COM PLAYWRIGHT
    # --------------------------------------------------------
    #
    # A versão de produção deve abrir o PGMEI oficial
    # através de navegador automatizado.
    #
    # Não deve:
    # - quebrar CAPTCHA;
    # - contornar autenticação;
    # - capturar credenciais de terceiros;
    # - utilizar endpoint não documentado como se fosse API.
    #
    # A sessão autenticada deve consultar:
    #
    # - competências;
    # - DAS emitido;
    # - DAS pago;
...
def consultar_dasn(cnpj, cpf=None, codigo_acesso=None):
    """
    Consulta da situação das declarações DASN-SIMEI.

    Não considera declaração entregue sem confirmação
    do portal oficial.
    """
...
st.title(
    "MEI Monitor V2 — Consulta e Regularidade"
)

st.caption(
    "Consultas baseadas nos portais oficiais "
    "do Simples Nacional / Receita Federal."
)

st.warning(
    "A aplicação nunca marcará DAS ou DASN como "
    "regular sem confirmação da fonte oficial."
)
...
with st.expander(
    "Dados de acesso para consultas autenticadas",
    expanded=False
):

    st.write(
        "Utilize somente credenciais autorizadas "
        "pelo responsável pelo CNPJ."
    )

    cpf_padrao = st.text_input(
        "CPF do responsável",
        type="password"
    )

    codigo_padrao = st.text_input(
        "Código de acesso",
        type="password"
    )

    st.info(
        "Esses dados não devem ser armazenados "
        "em planilhas de saída."
    )
...
if arquivo:
    ...
    if coluna_cnpj is None:
        ...
        st.error(
            "Não foi encontrada uma coluna contendo CNPJ."
        )

        st.stop()

    st.success(
        f"Coluna identificada: {coluna_cnpj}"
    )
    ...
    if st.button(
        ...
        for i, linha in df.iterrows():
            ...
            # Evita disparar consultas continuamente
            time.sleep(0.3)
...
st.caption(
    "Os resultados apresentados devem sempre identificar "
    "a fonte, data e hora da última consulta."
)
```
